...
import { FaPlay, FaUser, FaClock, FaBookOpen } from 'react-icons/fa';

const ProgramCard = ({ program, onClick, isExpertView = false }) => {
  const isEnrolled = program?.is_enroll || program?.status === 'InProgress' || program?.status === 'Complete' || isExpertView;
    ...
    >
      ...
      <div className="relative aspect-[16/9] overflow-hidden">
        <Image
          width={300}
          height={169}
          src={program.image || '/images/content/default.png'}
          alt={program.title}
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600">
          <div className="flex items-center gap-1">
            ...
            <span>{program.modules || 0} modules</span>
          </div>
          <div className="flex items-center gap-1">
            <FaPlay size={12} className="text-blue-500" />
            <span>{program.sessions || 0} sessions</span>
          </div>
        </div>
        
        {/* Experts */}
        <div className="flex items-center gap-2 text-sm text-gray-500 min-h-[1.25rem]">
          <FaUser size={12} className="text-purple-500" />
          <span className="line-clamp-1">
            {program.experts?.join(', ') || 'No experts assigned'}
          </span>
        </div>
