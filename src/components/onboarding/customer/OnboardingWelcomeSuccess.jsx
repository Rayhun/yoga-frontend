...
export default function OnboardingWelcomeSuccess({
  data,
  isPublic = false,
  ...
  const card = (
    <div className="box-border w-full min-w-0 rounded-3xl border border-gray-200/80 bg-white p-5 shadow-xl dark:border-gray-700 dark:bg-gray-900 sm:p-6 md:p-8">
      ...
      <div className="mt-8">
        <Button type="button" fullWidth onClick={handlePrimary} className="!rounded-full !py-3.5">
          {isPaymentComplete
            ? primaryAction?.label || 'Login'
            : primaryAction?.label || 'Go to community →'}
        </Button>
        {secondaryAction && !isPaymentComplete ? (
          <button
            type="button"
            onClick={handleSecondary}
            className="mt-3 w-full rounded-full px-4 py-2.5 text-sm font-medium text-gray-600 transition hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200"
          >
