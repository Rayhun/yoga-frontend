...
import StatisticsInfoButton from './StatisticsInfoButton';
import { HABIT_STATISTICS_HELP_DEFAULT } from './insightStatisticsDefaults';
import { getDailyInsights, getInsightStatisticsHelp } from '@/services/private/customer/goal';
...
const DailyInsights = () => {
  ...
  const { data: helpResponse } = useQuery({
    queryFn: () => getInsightStatisticsHelp('habit'),
    queryKey: ['insightStatisticsHelp', 'habit'],
  });

  const insightsData = insights?.data?.data;
  const habitHelp = helpResponse?.data?.data?.habit ?? HABIT_STATISTICS_HELP_DEFAULT;
        ...
        <div className="bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 text-white py-8 px-6 rounded-2xl shadow-2xl mb-6 relative overflow-hidden">
          ...
          <div className="relative z-10 flex justify-between items-center">
            <div className="flex items-center text-xl gap-3">
              ...
              <div>
                <h1 className="font-bold text-2xl">Habit Insights</h1>
                <p className="text-green-100 text-sm">Your daily habit analytics and patterns</p>
              </div>
            </div>
          ...
          <div className="mb-6 flex items-start justify-between gap-4">
            <div className="text-left">
              <h2 className="text-xl font-semibold text-gray-900 mb-2 flex items-center gap-2">
                ...
                {habitHelp?.title || 'Habits Statistics'}
              </h2>
              <p className="text-gray-600">Your current habit metrics and trends.</p>
            </div>
            <StatisticsInfoButton
              helpContent={habitHelp}
              hasData={false}
              insightType="habit"
            />
          </div>
        ...
        <div className="flex flex-col gap-8">
          <Section>
            <div className="mb-6 flex items-start justify-between gap-4">
              ...
              <StatisticsInfoButton helpContent={habitHelp} hasData={hasData} insightType="habit" />
            </div>
