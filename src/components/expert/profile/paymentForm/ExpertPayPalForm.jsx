...
const ExpertPayPalForm = ({ expertData }) => {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { user } = useAuthContext();

  const { mutateAsync: savePayPal } = useMutation({
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      await savePayPal({ payload: { paypal_email: values.paypal_email } });
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                Enter your PayPal email address to receive payments. Make sure this email is
                associated with an active PayPal account.
              </p>
            </div>
          </div>
        </div>
...
export default ExpertPayPalForm;
