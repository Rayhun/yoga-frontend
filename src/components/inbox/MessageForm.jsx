...
const Attachments = ({ selectedFiles, removeFile, isUploading, onClearAll }) => {
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200 p-4">
          ...
          <div className="flex flex-wrap gap-3">
            {selectedFiles.map((file, idx) => (
              <div key={`file-${idx}`} className="relative group animate-slideInUp" style={{ animationDelay: `${idx * 100}ms` }}>
                {isImageFile(file) ? (
                  ...
                  <div className="relative w-24 h-24 rounded-xl border-2 border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col items-center justify-center p-3 hover:border-green-300 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105">
                    <div className="text-3xl mb-1">{getFileIcon(file)}</div>
                    <div className="text-xs text-gray-600 text-center truncate w-full font-medium" title={file.name}>
...
const MessageForm = () => {
  ...
  const handleSendMessage = async () => {
    if (inputText.trim() !== '' || selectedFiles.length > 0) {
      ...
      try {
        ...
        sendMessage({ message: inputText, attachments: uploadedFiles });
  ...
  return (
    <div className="bg-white border-t border-gray-200 relative">
      <div className="p-3">
        <Attachments selectedFiles={selectedFiles} removeFile={removeFile} isUploading={isUploading} onClearAll={handleClearAll} />
