...
/**
 * Pill trigger opens a modal to search and multi-select LMS categories (IDs).
 */
const FormikCategoriesModalField = ({
  name,
  label,
  required,
  ...
  const { isLoading, isError, options } = useLMSCategoryOptions({ context, field: catalogField });

  const allRows = useMemo(
    () =>
      (options ?? [])
        .map(o => ({ id: Number(o.value), primaryLabel: String(o.label ?? '') }))
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!q) return allRows;
    return allRows.filter(r => r.primaryLabel.toLowerCase().includes(q));
  ...
  const idToLabel = useMemo(() => {
    const m = new Map();
    allRows.forEach(r => m.set(r.id, r.primaryLabel));
    return m;
  }, [allRows]);
          ...
          <div className="flex flex-wrap gap-1.5">
            {selectedIds.map(id => (
              <Chip
                key={id}
                size="small"
                label={idToLabel.get(id) ?? `Category #${id}`}
          ...
          <header className="flex shrink-0 items-start justify-between gap-3 px-5 pb-2 pt-5">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-bodydark2">
                Search and select one or more categories. Changes save as you tap.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="min-h-[240px] flex-1 overflow-y-auto px-3 py-2 sm:min-h-[280px]">
            {isError ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 py-16">
                ...
                <p className="text-sm text-gray-500 dark:text-bodydark2">Loading categories…</p>
              </div>
            ) : filteredRows.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
                <HiOutlineSquares2X2 className="h-14 w-14 shrink-0 text-gray-200 dark:text-bodydark2" />
...
export default FormikCategoriesModalField;
