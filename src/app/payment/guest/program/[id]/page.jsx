...
const Page = () => {
  ...
  const {
    ...
    queryFn: () => createGuestCheckoutSession({ type: 'program', object_id: Number(programId) || programId }),
    queryKey: [queryKeys.guestCheckoutSession, 'program', programId],
    ...
    <div className="flex flex-col gap-5">
      ...
      <LoadingWrapper isLoading={isLoading}>
        {clientSecret ? (
          ...
          <div className="text-center">Checkout Program Session not created properly</div>
        )}
      </LoadingWrapper>
    </div>
  );
